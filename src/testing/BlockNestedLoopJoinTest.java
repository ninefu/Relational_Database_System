...
public class BlockNestedLoopJoinTest extends BaseDatabaseTester {
	public static boolean HR = false; // This flag determines whether files will generated/output in human-readable format or in binary.
	...
	private void compareToTNLJ(String query, int pages) {
		PhysicalPlanConfig bnljConf = new PhysicalPlanConfig(JoinMethod.BNLJ, SortMethod.MEMORY, HR,IN); // Create the configuration file so that we will use BNLJ as the join method.
		bnljConf.setJoinBufferPages(pages);
		...
		PhysicalOperator bnljOp = getPhysicalPlan(query, bnljConf,null);
		PhysicalOperator tnljOp = getPhysicalPlan(query, tnljConf,null);
		dumpSortCompare(HR, tnljOp, bnljOp);
	}
...
//		testAgainstCases("test_cases", "2a", false);
//	}
//	
//	@Test
//	public void testCase2b() {
//		testAgainstCases("test_cases", "2b", false);
//	}
