...
public class SortMergeJoinAndExternalSortTester extends BaseDatabaseTester{
	...
	private void compareToTNLJ(String query, int pages) {
		PhysicalPlanConfig smjExternalConfig = new PhysicalPlanConfig(JoinMethod.SMJ, SortMethod.EXTERNAL, HR,IN); // Create the configuration file so that we will use BNLJ as the join method.
		smjExternalConfig.setSortBufferPages(pages);
		smjExternalConfig.setTempDir(".");
		...
		PhysicalOperator smjOp = getPhysicalPlan(query, smjExternalConfig,null);
